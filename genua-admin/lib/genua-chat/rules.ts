...
function getFlowStep(messages: ChatMessage[]): FlowStep {
  ...
  if (lastAssistantAsked(messages, 'sektorunuz') || lastAssistantAsked(messages, 'sektörünüz')) return 'company';
  if (lastAssistantAsked(messages, 'adın soyadın') || lastAssistantAsked(messages, 'telefon veya e-posta')) return 'contact';
  return 'idle';
}
...
function extractQuoteContext(messages: ChatMessage[]) {
  ...
  for (let i = 0; i < messages.length; i += 1) {
    ...
    if (prevN.includes('sektorunuz') || prevN.includes('sektörünüz')) company = msg.content.trim();
  }
...
export function buildScriptedReply(messages: ChatMessage[], userText: string): ChatReply {
  ...
  if (flow === 'contact') {
    ...
    if (!contact.contact && !contact.email && !contact.phone) {
      return { reply: 'İletişim için geçerli bir telefon veya e-posta yazar mısın? (Adınla birlikte)' };
    }
