...
try {
  console.log('instagram_reels migration uygulanıyor...');
  await sql.unsafe(query);
  const rows = await sql`SELECT COUNT(*)::int AS count FROM instagram_reels`;
  console.log('Tamam. instagram_reels tablosu hazır. Kayıt sayısı:', rows[0]?.count ?? 0);
} finally {
  await sql.end();
}
